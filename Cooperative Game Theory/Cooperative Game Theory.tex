...
\subsubsection*{배당}
Harsanyi는 다음 방식으로 협력 게임에서 한 연합의 시너지 효과를 고려했다.
...
\begin{sol}
	정리 \ref{thm:2.21}(c)를 이용하자. 임의의 순열 $\tau:N\to N$에 대하여
	\[
		\begin{aligned}
			\sum_{k=1}^n&\Phi_{\tau(k)}(\{\tau(1),\tau(2),\cdots,\tau(k)\},v)\\
			&=\sum_{k=1}^nD_{\tau(k)}P(\{\tau(1),\tau(2),\cdots,\tau(k)\},v)\\
			&=\sum_{k=1}^n[P(\{\tau(1),\tau(2),\cdots,\tau(k)\},v)-P(\{\tau(1),\tau(2),\cdots,\tau(k-1)\},v)]\\
			&=P(N,v)-P(\varnothing,v)=P(N,v)
		\end{aligned}
	\]
	이 성립하므로 주어진 식이 성립한다. (우변은 $\tau=\operatorname{id}$인 경우이다.)\qed
\end{sol}
...
\begin{sol}
	섀플리 값이 (i)--(iv)를 만족함은 자명하다. 역으로 $\mathcal G$ 위의 값 $\psi$가 (ii)--(iv)를 만족하면 $\psi$는 STD2를 만족함을 보이자. $(\{i,j\},v)\in\mathcal G$를 $2$인 협력 게임이라 하자. (ii)로부터
	\begin{equation}
		\label{eq:2.12}
		\psi_i(\{i,j\},v)+\psi_j(\{i,j\},v)=v(i,j)
	\end{equation}
	이다. 순열 $\sigma:\{i,j\}\to\{i,j\}$를 $\sigma(i)=j$, $\sigma(j)=i$로 정의하자. $v^\sigma(S)=v(\sigma^{-1}(S))$로 정의된 협력 게임 $v^\sigma$는
	\[
		v^\sigma(i)=v(j),\quad v^\sigma(j)=v(i),\quad v^\sigma(i,j)=v(i,j)
	\]
	을 만족한다. (iii)으로부터
	\begin{equation}
		\label{eq:2.13}
		\psi_i(\{i,j\},v^\sigma)=\psi_j(\{i,j\},v),\quad\psi_j(\{i,j\},v^\sigma)=\psi_i(\{i,j\},v)
	\end{equation}
	이 성립한다. 이때 $v^\sigma=v+(v(j)-v(i))u_i+(v(i)-v(j))u_j$이므로 (iv)로부터
	\begin{equation}
		\label{eq:2.14}
		\psi_i(\{i,j\},v^\sigma)=\psi_i(\{i,j\},v)+(v(j)-v(i))
	\end{equation}
	이 성립한다. 따라서 식 \ref{eq:2.13}과 \ref{eq:2.14}로부터
	\begin{equation}
		\label{eq:2.15}
		\psi_i(\{i,j\},v)=\psi_j(\{i,j\},v)+(v(i)-v(j))
	\end{equation}
	이 성립한다. 식 \ref{eq:2.12}와 \ref{eq:2.15}로부터
	\[
		\psi_i(\{i,j\},v)=\frac{v(i,j)+v(i)-v(j)}{2},\quad\psi_j(\{i,j\},v)=\frac{v(i,j)-v(i)+v(j)}{2}
	\]
	이다. $\psi$는 STD2를 만족하므로 정리 \ref{thm:2.26}로부터 $\psi=\Phi$이다.\qed
\end{sol}

\section{웨버 집합}
\subsection{웨버 집합}
앞서 살펴본 섀플리 값은 코어에 속할 필요도, 할당이 될 필요도 없다. 이 절에서는 섀플리 값을 확장한 해 집합, 웨버 집합을 고려한다.

\begin{dfn}{}{3.1}
	협력 게임 $(N,v)\in\mathcal G^N$의 \textbf{웨버 집합(Weber set)} $W(v)$는 $v$의 모든 한계 기여도 벡터 $m^\sigma(v)$의 볼록 껍질이다. 
\end{dfn}

이제 코어가 항상 웨버 집합의 부분집합임을 보이자.

\begin{lemma}{}{3.2}
	$Z\in\mathbb R^n$을 닫힌 볼록 집합이라 하고, $x\in\mathbb R^n\setminus Z$라 하자. 그러면 벡터 $y\in\mathbb R^n$이 존재하여 모든 $z\in Z$에 대하여 $y\cdot z>y\cdot x$이다.
\end{lemma}

\begin{thm}{}{3.3}
	협력 게임 $(N,v)\in\mathcal G^N$에 대하여 $C(v)\subseteq W(v)$이다.
\end{thm}

\begin{proof}
	$x\in C(v)\setminus W(v)$라 하자. 보조정리 \ref{lemma:3.2}로부터 벡터 $y\in\mathbb R^n$이 존재하여 모든 $w\in W(v)$에 대하여 $y\cdot w>y\cdot x$이다. $y_{\pi(1)}\geq y_{\pi(2)}\geq\cdots\geq y_{\pi(n)}$인 순열 $\pi\in\Pi(N)$을 고르자. $x\in C(v)$이므로
	\[
		\begin{aligned}
			y\cdot m^\pi&=\sum_{i=1}^ny_{\pi(i)}[v(P_\pi(\pi(i+1)))-v(P_\pi(\pi(i)))]\\
			&=y_{\pi(n)}v(N)-y_{\pi(1)}v(\varnothing)+\sum_{i=1}^{n-1}(y_{\pi(i)}-y_{\pi(i+1)})v(P_\pi(\pi(i+1)))\\
			&\leq y_{\pi(n)}\sum_{j=1}^nx_{\pi(j)}+\sum_{i=1}^{n-1}(y_{\pi(i)}-y_{\pi(i+1)})\sum_{j=1}^ix_{\pi(j)}\\
			&=\sum_{i=1}^ny_{\pi(i)}\sum_{j=1}^ix_{\pi(j)}-\sum_{i=2}^ny_{\pi(i)}\sum_{j=1}^{i-1}x_{\pi(j)}=\sum_{i=1}^ny_{\pi(i)}x_{\pi(i)}=y\cdot x
		\end{aligned}
	\]
	이 성립하나 이는 모순이다. 따라서 $C(v)\subseteq W(v)$이다.
\end{proof}

\begin{prob}{}{3.4}
	협력 게임 $(N,v)\in\mathcal G^N$의 한계 기여도 벡터 $m^\sigma$를 배당으로 표현하라.
\end{prob}

\begin{sol}
	정리 \ref{thm:2.15}로부터
	\[
		v(S)=\sum_{T\subseteq S}\Delta_v(T)
	\]
	이므로 각 $i\in N$에 대하여
	\[
		\begin{aligned}
			m^\sigma_i&=v(P_\sigma(i)\cup\{i\})-v(P_\sigma(i))\\
			&=\sum_{T\subseteq P_\sigma(i)\cup\{i\}}\Delta_v(T)-\sum_{T\subseteq P_\sigma(i)}\Delta_v(T)\\
			&=\sum_{T:i\in T\subseteq P_\sigma(i)\cup\{i\}}\Delta_v(T)
		\end{aligned}
	\]
	이다.\qed
\end{sol}
...
\subsection{볼록 게임}
협력 게임의 코어가 웨버 집합과 일치할 필요충분조건을 살펴보자.

\begin{dfn}{}{3.5}
	협력 게임 $(N,v)$가 모든 $S,T\subseteq N$에 대하여
	\begin{equation}
		\label{eq:3.1}
		v(S)+v(T)\leq v(S\cup T)+v(S\cap T)
	\end{equation}
	를 만족하면 $(N,v)$를 \textbf{볼록 게임(convex game)}이라 한다.
\end{dfn}

협력 게임의 볼록성은 초가법성을 함의한다. (식 \ref{eq:1.3} 참고) 두 개념은 모두 큰 연합이 상대적으로 더 많은 가치를 지닌다는 의미를 갖는다. 이 사고방식은 다음 보조정리에서도 드러난다.

\begin{lemma}{}{3.6}
	협력 게임 $(N,v)$가 볼록 게임일 필요충분조건은 다음과 같다.
	\begin{equation}
		\label{eq:3.2}
		\text{모든 }i\in N\text{와 }S\subseteq T\subseteq N\setminus\{i\}\text{에 대하여 }v(S\cup i)-v(S)\leq v(T\cup i)-v(T)\text{이다.}
	\end{equation}
\end{lemma}

\begin{proof}
	$v\in\mathcal G^N$이라 하자. 식 \ref{eq:3.1}에서 $S$와 $T$를 각각 $S\cup\{i\}$와 $T$로 대체하면 식 \ref{eq:3.2}를 얻는다. 역으로 식 \ref{eq:3.2}를 가정하자. 먼저 $S_0\subseteq T_0\subseteq N$, $R=\{i_1,\cdots,i_k\}\subseteq N\setminus T_0$라 하자. 식 \ref{eq:3.2}를 반복 적용하면
	\[
		\begin{aligned}
			v(S_0\cup \{i_1\})-v(S_0)&\leq v(T_0\cup \{i_1\})-v(T_0)\\
			v(S_0\cup\{i_1,i_2\})-v(S_0\cup i_1)&\leq v(T_0\cup\{i_1,i_2\})-v(T_0\cup i_1)\\
			&~~\vdots\\
			v(S_0\cup\{i_1,\cdots,i_k\})-v(S_0\cup\{i_1,\cdots,i_{k-1}\})&\leq v(T_0\cup\{i_1,\cdots,i_k\})-v(T_0\cup\{i_1,\cdots,i_{k-1}\}).
		\end{aligned}
	\]
	이고, 각 부등식을 변끼리 더하면
	\[
		v(S_0\cup R)-v(S_0)\leq v(T_0\cup R)-v(T_0)
	\]
	이다. 이제 임의의 $S,T\subseteq N$에 대하여 $S_0=S\cap T$, $T_0=T$, $R=S\setminus T$라 하면 식 \ref{eq:3.1}을 얻는다
\end{proof}

협력 게임의 볼록성으로 코어와 웨버 집합의 일치를 보장할 수 있다.

\begin{thm}{}{3.7}
	협력 게임 $v\in\mathcal G^N$가 볼록 게임일 필요충분조건은 $C(v)=W(v)$이다.
\end{thm}

\begin{proof}
	먼저 $v$가 볼록 게임이라 가정하자. 코어는 볼록 집합이므로 한계 기여도 벡터 $m^\sigma$가 코어에 속함을 보이면 충분하다. 편의를 위해 $\sigma=\operatorname{id}$라 하자. 연합 $S:=\{i_1,i_2,\cdots,i_k\}\subseteq N$에 대하여 $i_1\leq i_2\leq\cdots\leq i_k$라 하자. 그러면 보조정리 \ref{lemma:3.6}으로부터 모든 $1\leq k\leq s$에 대입하여
	\[
		v(i_1,\cdots,i_k)-v(i_1,\cdots,i{k-1})\leq v(1,2,\cdots,i_k)-v(1,2,\cdots,i_k-1)=m_{i_k}^\sigma
	\]
	이 성립한다. $k=1$부터 $k=s$까지의 부등식을 모두 더하면
	\[
		v(S)=v(i_1,\cdots i_k)\leq\sum_{k=1}^sm_{i_k}^\sigma=\sum_{i\in S}m_i^\sigma
	\]
	이므로 $m^\sigma\in C(v)$이다. 역으로 모든 한계 기여도 벡터가 코어에 속한다고 가정하자. 임의의 연합 $S,T\subseteq N$에 대하여 $N$의 참가자를 다음과 같이 구분하자:
	\[
		N=\{\underbrace{i_1,\cdots,i_k}_{S\cap T},\underbrace{i_{k+1},\cdots,i_l}_{T\setminus S},\underbrace{i_{l+1},\cdots,i_s}_{S\setminus T},\underbrace{i_{s+1},\cdots,i_n}_{N\setminus(S\cup T)}\}.
	\]
	자연스럽게 순열 $\tau\in\Pi(N)$을 $\tau(j)=i_j$로 정의하자. $m^\tau\in C(v)$이므로
	\[
		\begin{aligned}
			v(S)&\leq\sum_{i\in S}m_i^\tau=\sum_{j=1}^km_{i_j}^\tau+\sum_{j=l+1}^sm_{i_j}^\tau\\
			&=\sum_{j=1}^k[v(\{i_1,\cdots,i_j\})-v(\{i_1,\cdots,i_{j-1}\})]+\sum_{j=l+1}^s[v(\{i_1,\cdots,i_j\})-v(\{i_1,\cdots,i_{j-1}\})]\\
			&=[v(i_1,\cdots,i_k)-v(\varnothing)]+[v(i_1,\cdots,i_s)-v(i_1,\cdots,i_l)]\\
			&=v(S\cap T)+v(S\cup T)-v(T)
		\end{aligned}
	\]
	이 성립하고 $v$는 볼록 게임이다.
\end{proof}

정리 \ref{thm:3.7}과 섀플리 값의 정의(정의 \ref{dfn:2.2})로부터 다음을 얻는다.

\begin{cor}{}{3.8}
	볼록 게임 $v$의 섀플리 값은 코어에 속한다. 즉 $\Phi(v)\in C(v)$이다.
\end{cor}

\begin{prob}{}{3.9}
	협력 게임 $(N,v)$가 볼록 게임일 필요충분조건이 모든 $S\subseteq N$에 대하여 $v(S)=\min_{\sigma\in\Pi(N)}\sum_{i\in S}m_i^\sigma$임을 보여라.
\end{prob}

\begin{sol}
	연합 $S=\{i_1,i_2,\cdots,i_s\}$에 대하여 $N\setminus S=\{i_{s+1},\cdots,i_n\}$라 하고, 순열 $\sigma\in\Pi(N)$을 $\sigma(j)=i_j$로 정의하면 
	\[
		\sum_{i\in S}m_i^\sigma=\sum_{j=1}^s[v(i_1,\cdots,i_j)-v(i_1,\cdots,i_{j-1})]=v(S)
	\]
	이므로 정리 \ref{thm:3.7}에 의해 주어진 두 조건은 서로 필요충분조건이다.
\end{sol}

\begin{prob}{지주와 소작농의 수익 분배}{3.10}
	어느 국가에 농지를 소유한 지주(참가자 $0$)와 $n$명의 서로 동일한 노동력을 지닌 소작농(참가자 $1,2,\cdots,n$)이 있다. 지주는 자신의 땅을 농부들에게 임대하는 대신에 수확의 일부를 세금으로 받고자 한다. 소작농의 수에 따른 수확량을 함수 $f:\{0,1,\cdots,n\}\to\mathbb R$로 나타내자. $f$는 단조증가함수이고 $f(0)=0$이라 가정한다. 참가자 사이의 연합에 따른 수확량을 다음 협력 게임 $(N=\{0,1,\cdots,n\},v)$로 기술하자.
	\[
		v(S)=\begin{cases}
			0 & (0\notin S\text{인 경우})\\
			f(\vert S\vert-1) & (0\in S\text{인 경우})
		\end{cases}
	\]
	지주가 협력하지 않으면 농사를 지을 수 없으므로 수확량은 $0$이다. 반대로 지주가 농지를 제공하면 연합에 속한 소작농의 수에 따라 생산량이 결정된다.
	\begin{enumerate}[(a)]
		\item 이 협력 게임의 한계 기여도 벡터와 섀플리 값을 구하여라.
		\item 이 협력 게임이 볼록 게임일 필요충분조건을 $f$에 대한 조건으로 구하여라.
	\end{enumerate}
\end{prob}

\begin{sol}
	\begin{enumerate}[(a)]
		\item 먼저 지주 참가자 $0$의 섀플리 값을 계산하자. $\vert P_\sigma(0)\vert=k$인 순열 $\sigma\in\Pi(N)$에 대하여 $0\notin P_\sigma(0)$이므로
			\[
				m_0^\sigma=v(P_\sigma(0)\cup0)-v(P_\sigma(0))=f(k)
			\]
			이다. $\vert P_\sigma(0)\vert=k$인 순열 $\sigma$의 수는 $n!$이므로 $\Phi_0(v)$는 다음과 같다.
			\[
				\Phi_0(v)=\frac{1}{(n+1)!}\sum_{k=0}^nn!f(k)=\frac{1}{n+1}\sum_{k=0}^nf(k)
			\]
			이제 소작농 참가자 $i$의 섀플리 값을 계산하자. 만약 $0\notin P_\sigma(i)$이면 $m_i^\sigma=0$이다. $0\in P_\sigma(i)$이고 $\vert P_\sigma(i)\vert=k$인 순열 $\sigma\in\Pi(N)$에 대하여
			\[
				m_i^\sigma=v(P_\sigma(i)\cup\{i\})-v(P_\sigma(i))=f(k)-f(k-1)
			\]
			이다. $0\in P_\sigma(i)$이고 $\vert P_\sigma(i)\vert=k$인 순열 $\sigma$의 수는 $k(n-1)!$이므로 $\Phi_i(v)$는 다음과 같다.
			\[
				\begin{aligned}
					\Phi_i(v)&=\frac{1}{(n+1)!}\sum_{k=1}^nk(n-1)![f(k)-f(k-1)]\\
					&=\frac{1}{n(n+1)}\sum_{k=1}^nk[f(k)-f(k-1)]\\
					&=\frac{1}{n(n+1)}\sum_{k=1}^n[((k+1)f(k)-kf(k-1))-f(k)]\\
					&=\frac{1}{n(n+1)}\left((n+1)f(n)-f(0)-\sum_{k=1}^nf(k)\right)\\
					&=\frac{1}{n}\left(f(n)-\frac{1}{n+1}\sum_{k=0}^nf(k)\right)=\frac{f(n)-\Phi_0(v)}{n}
				\end{aligned}
			\]
		\item $i\in N\setminus\{0\}$이라 하자. $0\in S\subseteq T\subseteq N\setminus\{i\}$에 대하여
			\[
				v(S\cup\{i\})-v(S)=f(\vert S\vert)-f(\vert S\vert-1),\quad v(T\cup\{i\})-v(T)=f(\vert T\vert)-f(\vert T\vert-1)
			\]
			이므로 보조정리 \ref{lemma:3.6}로부터 협력 게임 $v$가 볼록 게임일 필요충분조건은 $k=1,2,\cdots,n-1$에 대하여
			\[
				f(k)-f(k-1)\leq f(k+1)-f(k),
			\]
			즉 $f$가 볼록 함수인 것이다.
	\end{enumerate}
\end{sol}
...
\begin{remark}
	소작농 참가자는 $v$에서 서로 대칭이므로 EFF와 SYM으로 소작농 참가자의 섀플리 값을 쉽게 구할 수 있다.
\end{remark}

\begin{prob}{활주로 건설 비용 분담}{3.11}
	$n$개의 항공사가 자신들이 이용할 활주로를 건설하고자 한다. 항공사 $i$의 단독 활주로 건설 비용은 필요한 활주로 길이에 따라 $c_i$로 책정하였다. $0\leq c_1\leq c_2\leq\cdots\leq c_n$이라 가정한다. 항공사가 연합하여 활주로를 공동으로 건설할 때 절약되는 비용을 다음 협력 게임 $(N=\{1,\cdots,n\},v)$로 기술하자.
	\[
		v(S)=\sum_{i\in S}c_i-\max_{i\in S}c_i
	\]
	항공사들이 연합하면 가장 긴 활주로 하나만 건설하여 공유하고, 각자 단독으로 건설할 때보다 비용을 절약할 수 있다.
	\begin{enumerate}[(a)]
		\item 이 협력 게임이 볼록 게임임을 보여라.
		\item 이 협력 게임의 한계 기여도 벡터, 섀플리 값, 코어를 구하여라.
	\end{enumerate}
\end{prob}

\begin{sol}
	\begin{enumerate}[(a)]
		\item 임의의 $i\in N$과 $S\subseteq T\subseteq N\setminus\{i\}$에 대하여
			\[
				\begin{aligned}
					v(S&\cup i)-v(S)-v(T\cup i)+v(T)\\
					&=\biggl(\sum_{j\in S\cup i}c_j-\sum_{j\in S}c_j-\sum_{j\in T\cup i}c_j+\sum_{j\in T}c_j\biggr)-\biggl(\max_{j\in S\cup i}c_j-\max_{j\in S}c_j-\max_{j\in T\cup i}c_j+\max_{j\in T}c_j\biggr)\\
					&=\max_{j\in T\cup i}c_j+\underbrace{\max_{j\in S}c_j}_{=:A}-\max_{j\in S\cup i}c_j-\underbrace{\max_{j\in T}c_j}_{=:B}\\
					&=(\max\{B,c_i\}+A)-(\max\{A,c_i\}+B)\\
					&=[(A+B+c_i)-\min\{B,c_i\}]-[(A+B+c_i)-\min\{A,c_i\}]\\
					&=\min\{A,c_i\}-\min\{B,c_i\}\leq0
				\end{aligned}
			\]
			이 성립하므로 보조정리 \ref{lemma:3.6}으로부터 $v$는 볼록 게임이다. (마지막 부등식은 $A\leq B$이므로 성립한다.)
		\item $i\in N$, $\sigma\in\Pi(N)$이라 하자.
			\[
				\begin{aligned}
					m_i^\sigma&=v(P_\sigma(i)\cup i)-v(P_\sigma(i))\\
					&=\sum_{j\in P_\sigma(i)\cup i}c_j-\sum_{j\in P_\sigma(i)}c_j+\max_{j\in P_\sigma(i)}c_j-\max_{j\in P_\sigma(i)\cup i}c_j\\
					&=c_i+\max_{j\in P_\sigma(i)}c_j-\max_{j\in P_\sigma(i)\cup i}c_j
				\end{aligned}
			\]
			\begin{enumerate}[(i)]
				\item 어떤 $j\in P_\sigma(i)$에 대하여 $j>i$이면 $\max_{j\in P_\sigma(i)\cup i}c_j=\max_{j\in P_\sigma(i)}c_j$이므로 $m_i^\sigma=c_i$이다.
				\item 모든 $j\in P_\sigma(i)$에 대하여 $j<i$이면 $\max_{j\in P_\sigma(i)\cup i}c_j=c_i$이므로 $m_i^\sigma=\max_{j\in P_\sigma(i)}c_j$이다.
			\end{enumerate}
			먼저 (i)을 만족하지 않는 순열 $\sigma$의 개수를 구하자. 이는 순열에서 집합 $N_{\geq i}:=\{i,i+1,\cdots,n\}$의 원소만을 보았을 때 $i$가 먼저 등장하는 경우의 수이다. 따라서 구하는 순열의 수는
			\[
				(n-i)!\times\binom{n}{n-i+1}\times(i-1)!=n!\times\frac{1}{n-i+1}
			\]
			이다. 다음으로 $j<i$에 대하여 $m^\sigma=c_j$가 되도록 하는 순열 $\sigma$의 개수를 구하자. 이는 순열에서 집합 $N_{\geq j}:=\{j,j+1,\cdots,i,\cdots,n\}$의 원소만을 보았을 때 $j$, $i$가 이 순서대로 먼저 등장하는 경우의 수이다. 따라서 구하는 순열의 수는
			\[
				(n-j-1)!\times\binom{n}{n-j+1}\times(j-1)!=n!\times\frac{1}{(n-j+1)(n-j)}
			\]
			이다. 이제 섀플리 값을 계산하자.
			\[
				\begin{aligned}
					\Phi_i(v)&=\frac{1}{n!}\biggl[n!\biggl(1-\frac{1}{n-i+1}\biggr)+\sum_{j=1}^{i-1}\frac{n!}{(n-j+1)(n-j)}c_j\biggr]\\
					&=\biggl(1-\frac{1}{n-i+1}\biggr)c_i+\sum_{j=1}^{i-1}\frac{1}{(n-j+1)(n-j)}c_j\\
				\end{aligned}
			\]
			에서 $c_0=0$이라 하고 Abel의 부분합 공식을 적용하면
			\[
				\sum_{j=1}^{i-1}\frac{1}{(n-j+1)(n-j)}c_j=\frac{1}{n-i+1}c_{i-1}-\sum_{j=1}^{i-1}\frac{c_j-c_{j-1}}{n-j+1}
			\]
			이므로
			\[
				\begin{aligned}
					\Phi_i(v)&=\biggl(1-\frac{1}{n-i+1}\biggr)c_i+\frac{1}{n-i+1}c_{i-1}-\sum_{j=1}^{i-1}\frac{c_j-c_{j-1}}{n-j+1}\\
					&=c_i-\sum_{j=1}^{i}\frac{c_j-c_{j-1}}{n-j+1}
				\end{aligned}
			\]
			이다. $v$가 볼록 게임이므로 정리 \ref{thm:3.7}에 의해 $v$의 코어는 한계 기여도 벡터의 볼록 껍질이다.\qed
	\end{enumerate}
\end{sol}
